...
def EigenvalueRecurrentNN_withScaling(n_days, hidden_layer_sizes, recurrent_layer_sizes,  post_recurrent_layer_sizes=[],
    ...
    rescaled_returns, alphas, betas, gammas = cl.Return_Weightings(
        name='ReturnRescaling', demean=(target_return != None))(inputs)
    ...
    if apply_on_correlation == True:
        ...
        std, mean = cl.StandardDeviationLayer(
            axis=-1, demean=(target_return == None), name='SampleSTD')(rescaled_returns)
    ...
    if hidden_layer_sizes[0] > 0 and apply_on_correlation == False:
        raise ValueError(
            "hidden_layer_sizes can only be used with apply_on_correlation=True.")
        ...
        std = cl.CustomNormalizationLayer(
            axis=-2, mode='sum', name='STDNormalization')(std)
        TransformedStd = 1/std

    TransformedCorrelationMatrix = cl.EigenProductLayer(
        scaling_factor='inverse', name='TransformedCorrComposition')(TransformedEigenvalues, EigenVectors)

    if apply_on_correlation == True:
        TransformedCovarianceMatrix = TransformedCorrelationMatrix * \
            cl.CovarianceLayer(
                normalize=False, name='AddTransformedSTD')(TransformedStd)
    else:
        TransformedCovarianceMatrix = TransformedCorrelationMatrix

    weights = cl.NormalizedSum(
        axis_1=-1, axis_2=-2, name='PtfWeights')(TransformedCovarianceMatrix)
...
def Best_Cov(n_days, hidden_layer_sizes, recurrent_layer_sizes,  post_recurrent_layer_sizes=[],
    ...
    if apply_on_correlation == True:
        ...
        scaling_factor = 'none'
    TransformedCorrelationMatrix = cl.EigenProductLayer(
        scaling_factor=scaling_factor, name='TransformedCorrComposition')(TransformedEigenvalues, EigenVectors)
...
def SimpleModel(n_days, hidden_layer_sizes=[8], recurrent_layer_sizes=[32],  post_recurrent_layer_sizes=[],
    ...
    # Input shape can be adjusted as needed
    ...
    EigenValues = cl.DimensionAwareLayer(features=dimensional_features, name='DimensionAware')([
        EigenValues, inputs])  # rsqrt_n_days
    ...
    if len(hidden_layer_sizes) > 0:
        TransformedStd = cl.DeepLayer(hidden_layer_sizes+[1],
                                      last_activation='softplus', name='STDTansformation')(std)
        TransformedStd = cl.CustomNormalizationLayer(axis=-2, mode=normalize_std,
                                                     name='STDInverseNormalization')(TransformedStd)
    else:
        TransformedStd = 1./std
